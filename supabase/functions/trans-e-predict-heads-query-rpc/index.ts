...
/**
 * Head予測関数
 * 与えられたrelationとtailから、最も可能性の高いheadを予測します
 *
 * 入力パラメータ:
 * - tail: エンティティID（tail）
 * - relation: リレーションタイプ
...
Deno.serve(
  async (req: { json: () => PromiseLike<{ query: any }> | { query: any } }) => {
    const { tail, relation, topicSpaceId } = await req.json();

    console.log(
      `Predicting heads for relation: ${relation}, tail: ${tail}, topicSpace: ${topicSpaceId}`,
    );
    ...
    // headを予測（relationとtailからheadを予測）
    const heads = predictor.predictHead(relation, tail, 10);

    console.log(
      `Predicted ${heads.length} heads for relation "${relation}" and tail "${tail}":`,
      heads,
    );

    return new Response(JSON.stringify(heads), {
...
/* To invoke locally:
  ...
  curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/trans-e-predict-heads-query-rpc' \
    --header 'Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZS1kZW1vIiwicm9sZSI6ImFub24iLCJleHAiOjE5ODM4MTI5OTZ9.CRXP1A7WOeoJeXxjNni43kdQwgnWNReilDMblYTn_I0' \
    --header 'Content-Type: application/json' \
    --data '{"tail":"entity1","relation":"RELATED_TO","topicSpaceId":"space1"}'
