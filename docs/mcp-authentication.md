...
Drive 連携済みリポジトリでは、4. `sync_topic_space_drive_folder` でフォルダ同期（詳細は [Google Drive 同期](./topic-space-drive-sync.md)）。

統合グラフのドキュメント対応（provenance）は [TopicSpace ノード・エッジ provenance](./topic-space-node-provenance.md) を参照。
...
- **platform** (`/api/mcp`) — 議事録取り込み、TopicSpace 作成・attach/detach、Drive 同期、変更履歴 replay
