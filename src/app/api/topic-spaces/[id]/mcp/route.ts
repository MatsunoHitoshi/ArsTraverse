...
class DraftAuthRequiredError extends Error {
  constructor() {
    super("Draft authentication required");
    this.name = "DraftAuthRequiredError";
  }
}

function requireDraftCtx(
  draftCtx: McpDraftHandlerCtx | null,
): McpDraftHandlerCtx {
  if (!draftCtx) {
    throw new DraftAuthRequiredError();
  }
  return draftCtx;
}
...
function formatMcpToolError(
  ...
  if (error instanceof DraftAuthRequiredError) {
    return {
      content: [
        { type: "text", text: getMcpMessage(locale, "draftAuthRequired") },
      ],
    };
  }
  console.error(error);
      ...
      server.tool(
        ...
        async ({ queries }) => {
          try {
            ...
            if (results.length === 0) {
              const queryText = queries.map((query) => `"${query}"`).join(", ");
              return {
                content: [
                  {
                    type: "text",
                    text:
                      locale === "en"
                        ? `${getMcpMessage(locale, "searchNoResults")}${queryText}.`
                        : `「${queryText}」${getMcpMessage(locale, "searchNoResults")}`,
                  },
                ],
              };
            }
            const textResponse =
              getMcpMessage(locale, "searchResultsHeader") +
              "\n" +
              results
                .map(
                  (result) =>
            ...
            return {
              content: [
                { type: "text", text: getMcpMessage(locale, "searchError") },
              ],
            };
          }
        },
          ...
          try {
            ...
            switch (true) {
              ...
              case results.graphData.nodes.length === 0:
                textResponse =
                  locale === "en"
                    ? `${getMcpMessage(locale, "relationNotFound", { start: startNodeName, end: endNodeName })}\n\n`
                    : `${startNodeName}と${endNodeName}${getMcpMessage(locale, "relationNotFound")}\n\n`;
            ...
            return {
              content: [
                {
                  type: "text",
                  text: getMcpMessage(locale, "relationFetchError"),
                },
              ],
            };
          }
        ...
        async ({ nodeId }) => {
          try {
            ...
            if (result.relatedNodes.length > 0) {
              ...
              textResponse += getMcpMessage(locale, "noRelatedInfo");
            }
            ...
            return {
              content: [
                {
                  type: "text",
                  text: getMcpMessage(locale, "contextFetchError"),
                },
              ],
            };
          }
          ...
          try {
            ...
            return {
              content: [
                {
                  type: "text",
                  text: getMcpMessage(locale, "referenceFetchError"),
                },
              ],
            };
          }
          ...
          try {
            ...
            return {
              content: [
                {
                  type: "text",
                  text: getMcpMessage(locale, "listGraphError"),
                },
              ],
            };
          }
          ...
          try {
            ...
            if (embeddingCandidates.length === 0) {
              if (!userAuthToken) {
                embeddingSkippedReason =
                  getMcpMessage(locale, "embeddingAuthRequired");
              } else if (!process.env.NEXT_PUBLIC_SUPABASE_URL) {
                embeddingSkippedReason = getMcpMessage(
                  locale,
                  "embeddingUrlMissing",
                );
              }
            ...
            return {
              content: [
                {
                  type: "text",
                  text: getMcpMessage(locale, "findDuplicateNodesError"),
                },
              ],
            };
          }
...
アライメント作業の起点として、${findDuplicateNodeCandidatesToolName} より先に使うことを推奨します。`,
          ...
          try {
            ...
            return {
              content: [
                {
                  type: "text",
                  text: getMcpMessage(locale, "findExactDuplicateGroupsError"),
                },
              ],
            };
          }
          ...
          try {
            ...
            return {
              content: [
                {
                  type: "text",
                  text: getMcpMessage(locale, "findDuplicateEdgesError"),
                },
              ],
            };
          }
          ...
          try {
            ...
            return {
              content: [
                {
                  type: "text",
                  text: getMcpMessage(locale, "getLabelDistributionError"),
                },
              ],
            };
          }
      ...
      const propertiesRecordSchema = z.record(propertyValueSchema).optional();
...
マージ/承認は Admin/UI の既存フローに委ねてください。`,
        ...
        async ({ title, description }) => {
          try {
            const proposal = await mcpCreateDraftProposal(requireDraftCtx(draftCtx), {
              topicSpaceId,
              title,
              description,
            });
            ...
            return formatMcpToolError(
              error,
              getMcpMessage(locale, "createDraftError"),
              locale,
            );
          }
        },
...
アライメント後の統合結果を確認する際に利用してください。`,
        ...
        async ({ proposalId }) => {
          try {
            const result = await mcpGetProposalDraftGraph(requireDraftCtx(draftCtx), proposalId);
            ...
            return formatMcpToolError(
              error,
              getMcpMessage(locale, "getDraftGraphError"),
              locale,
            );
          }
        },
...
${submitGraphEditProposalToolName} の前に必ず呼び出し、変更内容を確認してください。hasChanges が false の場合は提出しないでください。`,
        ...
        async ({ proposalId }) => {
          try {
            const result = await mcpGetProposalDraftDiff(requireDraftCtx(draftCtx), proposalId);
            ...
            return formatMcpToolError(
              error,
              getMcpMessage(locale, "getDraftDiffError"),
              locale,
            );
          }
        },
...
ノードのpropertiesは任意のオブジェクトで、数値/真偽/ null も渡せます（tRPC側で文字列化されます）。`,
        ...
        async ({ proposalId, nodeId, name, label, properties }) => {
          try {
            await mcpUpsertNodeInDraft(requireDraftCtx(draftCtx), {
              proposalId,
              node: {
                id: nodeId,
                name,
            ...
            return formatMcpToolError(
              error,
              getMcpMessage(locale, "upsertNodeError"),
              locale,
            );
          }
        },
...
削除後は同じ nodeId を再利用せず、必要なら別nodeIdで ${upsertNodeToolName} してください。`,
        ...
        async ({ proposalId, nodeId }) => {
          try {
            await mcpDeleteNodeInDraft(requireDraftCtx(draftCtx), {
              proposalId,
              nodeId,
            });
            return {
            ...
            return formatMcpToolError(
              error,
              getMcpMessage(locale, "deleteNodeError"),
              locale,
            );
          }
        },
...
事前に ${upsertNodeToolName} で nodeId を登録しておいてください（未登録だとエラーになります）。`,
        ...
        async ({ proposalId, nodeId, key, value }) => {
          try {
            await mcpSetNodePropertyInDraft(requireDraftCtx(draftCtx), {
              proposalId,
              nodeId,
              key,
              value,
            ...
            return formatMcpToolError(
              error,
              getMcpMessage(locale, "setNodePropertyError"),
              locale,
            );
          }
        },
...
キーが存在しない場合は no-op です。`,
        ...
        async ({ proposalId, nodeId, key }) => {
          try {
            await mcpUnsetNodePropertyInDraft(requireDraftCtx(draftCtx), {
              proposalId,
              nodeId,
              key,
            });
            ...
            return formatMcpToolError(
              error,
              getMcpMessage(locale, "unsetNodePropertyError"),
              locale,
            );
          }
        },
...
propertiesは任意です。`,
        ...
        async ({ proposalId, edgeId, type, sourceId, targetId, properties }) => {
          try {
            await mcpUpsertRelationshipInDraft(requireDraftCtx(draftCtx), {
              proposalId,
              relationship: {
                id: edgeId,
                type,
            ...
            return formatMcpToolError(
              error,
              getMcpMessage(locale, "upsertEdgeError"),
              locale,
            );
          }
        },
...
削除後は同じ edgeId を再利用せず、必要なら別edgeIdで ${upsertEdgeToolName} してください。`,
        ...
        async ({ proposalId, edgeId }) => {
          try {
            await mcpDeleteRelationshipInDraft(requireDraftCtx(draftCtx), {
              proposalId,
            ...
            return formatMcpToolError(
              error,
              getMcpMessage(locale, "deleteEdgeError"),
              locale,
            );
          }
        },
...
事前に ${upsertEdgeToolName} で edgeId を登録しておいてください（未登録だとエラーになります）。`,
        ...
        async ({ proposalId, edgeId, key, value }) => {
          try {
            await mcpSetRelationshipPropertyInDraft(requireDraftCtx(draftCtx), {
              proposalId,
              relationshipId: edgeId,
              key,
              value,
            ...
            return formatMcpToolError(
              error,
              getMcpMessage(locale, "setEdgePropertyError"),
              locale,
            );
          }
        },
...
キーが存在しない場合は no-op です。`,
        ...
        async ({ proposalId, edgeId, key }) => {
          try {
            await mcpUnsetRelationshipPropertyInDraft(requireDraftCtx(draftCtx), {
              proposalId,
              relationshipId: edgeId,
              key,
            });
            ...
            return formatMcpToolError(
              error,
              getMcpMessage(locale, "unsetEdgePropertyError"),
              locale,
            );
          }
        },
...
推奨フロー: ${createDraftProposalToolName} → ${findDuplicateNodeCandidatesToolName} で候補確認 → 本ツール → ${getDraftGraphToolName} で確認。`,
        ...
        async ({
          ...
          try {
            const result = await mcpMergeNodesInDraft(requireDraftCtx(draftCtx), {
              proposalId,
              canonicalNodeId,
              duplicateNodeIds,
              canonicalName,
            ...
            return formatMcpToolError(
              error,
              getMcpMessage(locale, "mergeNodesError"),
              locale,
            );
          }
        },
      ...
      server.tool(
        ...
        async ({ proposalId, edgeGroups }) => {
          try {
            ...
            return formatMcpToolError(
              error,
              getMcpMessage(locale, "deduplicateEdgesError"),
              locale,
            );
          }
        },
      ...
      server.tool(
        ...
        async ({ proposalId }) => {
          try {
            const diff = await mcpGetProposalDraftDiff(requireDraftCtx(draftCtx), proposalId);
            ...
            return formatMcpToolError(
              error,
              getMcpMessage(locale, "submitProposalError"),
              locale,
            );
          }
        },
...
const routeHandler = async (
  ...
  if (!topicSpaceId) {
    return new Response(getMcpMessage(locale, "topicSpaceIdMissing"), {
      status: 400,
    });
  }
  let topicSpaceInfo;
  try {
    ...
    if (error instanceof TRPCError && error.code === "NOT_FOUND") {
      return new Response(getMcpMessage(locale, "topicSpaceNotFound"), {
        status: 404,
      });
    }
    throw error;
