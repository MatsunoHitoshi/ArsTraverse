      ...
      async ({ name, plainText }) => {
        try {
          ...
          return formatMcpToolError(
            error,
            "SourceDocument の作成中にエラーが発生しました。",
          );
        }
      },
    );
...
mergeOnly=true で統合履歴のみに絞れます。before に ISO 日時を指定すると Detach/Attach より前の履歴だけ取得できます。`,
      {
        ...
        before: z
          .string()
          .optional()
          .describe("この日時より前の履歴のみ（ISO 8601）"),
        after: z
          .string()
          .optional()
          .describe("この日時より後の履歴のみ（ISO 8601）"),
...
dryRun=true で適用せずに確認できます。before でバックフィル開始日時より前の統合だけ対象にできます。`,
      {
        ...
        before: z
          .string()
          .optional()
          .describe("この日時より前の統合履歴のみ再適用（ISO 8601）"),
        after: z
          .string()
          .optional()
          .describe("この日時より後の統合履歴のみ再適用（ISO 8601）"),
        changeHistoryIds: z
          .array(z.string().min(1))
          .optional()
...
const routeHandler = async (request: NextRequest) => {
  ...
  const handler = createPlatformMcpHandler(mcpAuth.auth, mcpAuth.draftCtx);
