  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto bg-slate-900 p-4">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-sm font-semibold text-slate-200">{t("nodeInfo")}</h2>
        {onClose && (
          <button
            onClick={onClose}
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-2">
          ...
          <div className="text-sm text-slate-400">
            {t("labelWithValue", { label: node.label })}
          </div>
        ...
        {Object.keys(node.properties ?? {}).length > 0 ? (
          ...
          <div className="text-sm text-slate-400">{t("noProperties")}</div>
        )}
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-col gap-1 text-sm text-slate-400">
            <div>
              {t("connectionCount", {
                count: node.neighborLinkCount ?? 0,
              })}
            </div>
          </div>
        </div>
