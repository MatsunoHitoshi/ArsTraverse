...
const TEXT_MODE_PROCESSING_STEPS = [
  "本文からグラフを抽出しています",
  "リポジトリに統合しています",
  "クラスタリングを実行しています",
] as const;
  ...
  processingMode = null,
    ...
    return (
      <Modal
        ...
        title={showProcessing ? "ストーリーを生成しています" : "ストーリーの生成方法を選択"}
        size="large"
      >
          ...
          {showProcessing ? (
            <>
              <p className="text-sm text-slate-300">
                本文からグラフを抽出し、リポジトリに統合してクラスタリングしています。しばらくお待ちください。
              </p>
              <div className="flex flex-col gap-3 py-4">
                {TEXT_MODE_PROCESSING_STEPS.map((step, idx) => (
                  <div
                    key={step}
              ...
              <p className="text-xs text-slate-500">
                完了すると自動で閉じます。グラフ抽出に時間がかかる場合があります。
              </p>
            </>
          ) : (
            <>
