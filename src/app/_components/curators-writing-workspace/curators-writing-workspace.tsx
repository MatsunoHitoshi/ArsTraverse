...
import { ResizableSplitLayout } from "../layout/resizable-split-layout";
...
const normalizeTiptapContent = (value: unknown): unknown => {
  if (Array.isArray(value)) {
    return value
      .map((item) => normalizeTiptapContent(item))
  ...
  for (const [key, child] of Object.entries(record)) {
    ...
    const normalizedChild = normalizeTiptapContent(child);
...
const CuratorsWritingWorkspace = ({
  ...
  const [editorContent, setEditorContent] = useState<JSONContent>(
    ((workspace.content
      ? normalizeTiptapContent(workspace.content)
      : undefined) as JSONContent) ?? DEFAULT_CONTENT,
  );
  ...
  const defaultLeftRatio =
    isGraphEditor || isGraphSelectionMode || isStorytellingMode ? 0.5 : 2 / 3;
  ...
  return (
    <div className="flex h-screen w-full bg-slate-900 p-4 font-sans">
      <ResizableSplitLayout
        isRightOpen={isRightPanelOpen}
        defaultLeftRatio={defaultLeftRatio}
        minLeftRatio={0.35}
        maxLeftRatio={0.8}
        className="h-[calc(100svh-80px)] gap-1"
        leftPaneClassName="min-w-0"
        rightPaneClassName="min-w-0"
          ...
          {isStorytellingMode &&
            ...
            (metaGraphStory.generationMode === null
              ...
              <StoryGenerationModeModal
                ...
                hasReferencedTopicSpace={Boolean(topicSpaceId)}
